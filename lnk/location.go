...
func parseLocationInfo(header *Header, offset int64, r io.ReaderAt) (*Location, int64, error) {
	...
	if hasFlag(flags, volumeIDAndLocalBasePath) {
		localBasePath = internal.ReadString(data, int(localBasePathOffset))
	...
	commonPathSuffix := internal.ReadString(data, int(commonOffset))
...
func parseVolumeInfo(data []byte) (*Volume, error) {
	...
	if hasUnicodeLabel {
		...
		volumeLabel = internal.ReadUnicode(data, int(volumeLabelOffset))
	} else {
		volumeLabel = internal.ReadString(data, int(volumeLabelOffset))
	}
...
func parseNetworkShareInfo(data []byte) (*NetworkShare, error) {
	...
	shareName := internal.ReadString(data, int(shareNameOffset))
	var deviceName string
	if hasFlag(flags, validDevice) {
		deviceName = internal.ReadString(data, int(deviceNameOffset))
	}
	return &NetworkShare{
		Name:         shareName,
